...
#debug
force_masscalc=False

composition_file="D:/231225_CartMFP/CartMFP_test/C[72]H[134]N[15]P[3]O[30]S[5]_b100000max1000rdbe-5_39_7grCommon_comp.npy"
...
def predict_formula(
    ...
    if not os.path.exists(mass_index_file):   mass_index_file=mass_index_file[:-1]+"z"             #sparse index
    ...
    if os.path.exists(params_file) and use_params:
        ...
        elements,fs,mass_blowup,max_mass=[jdf[i] for i in ["elements","composition","mass_blowup","max_mass"]]
        ...
        max_mass=float(composition_file.split("max")[1].split("rdbe")[0])
        
    # Parse RDBE information
    ...
    if len(adducts):
        ...
        #handle mass based adducts
        adduct_mass=[]
        for a in adducts:
            if a[1] in mdf.index: am=getMz(a[1:]) #check if elemet or digit
            else:
                if   a[-1]=="+": am=float(a[1:-1])-emass
                elif a[-1]=="-": am=float(a[1:-1])+emass
                else           : am=float(a[1:-1])
            adduct_mass.append(am)
        adduct_mass=np.array(adduct_mass)
    ...
    #load mass file
    if mass_index_file[-1]=="z":
        from scipy import sparse 
        emp = sparse.load_npz(mass_index_file) #sparse
    else:
    ...
    #total solutions
    sols=emp[um,1]
    if mass_index_file[-1]=="z": sols=np.array(sols.todense()) #sparse
    tot_sols=pd.DataFrame(np.vstack([a_ix,sols.flatten()]).T,columns=["index","solutions"]).groupby("index")["solutions"].sum()
    ...
    if pre_filter_mass:
        ...
        if mass_index_file[-1]=="z":
            t=np.array([top_candidates]*len(pmi))-np.array(emp[pmi,1].todense()).flatten()
        else:
        ...
        while any(tog): 
            ...
            if mass_index_file[-1]=="z":
                r=np.array(emp[pmi+c,1].todense()).flatten()
                l=np.array(emp[pmi-c,1].todense()).flatten()
            else:
                r=emp[pmi+c,1]
                l=emp[pmi-c,1]
    ...
    if mass_index_file[-1]=="z": x=np.array(x.todense()) #convert sparse 
    ...
    if not os.path.exists(mass_defect_file) or force_masscalc:
        ms=np.sum(cs*mdf.loc[elements].values.T,axis=1)                                  #direct float mass calculation
    else:  ms = np.load(mass_defect_file, mmap_mode="r")[cq] #precomputed float mass 
    ...
    if len(ms):
        ...
        if len(mt): 
            ...
            #unique formulas
            cqu,cqx,cqi=np.unique(cq,return_index=True,return_inverse=True)
            ms,cs=ms[cqx],cs[cqx]
            
            tree = KDTree(ms.reshape(1,-1).T, leaf_size=200)  #should be unique masses?
            ...
            f_us=np.hstack([f_us,np.repeat(mt,top_candidates)])

        res=pd.DataFrame(mass_df.iloc[f_us,:])
        res["pred_mass"]=f_ms
        res["total_solutions"]=tot_sols.loc[res["index"].values].values
        ...
        res["rdbe"]=(  f_cs[:, Xrdbe].sum(axis=1)*2
                      -f_cs[:, Yrdbe].sum(axis=1)
                      +f_cs[:, Zrdbe].sum(axis=1))/2+1
