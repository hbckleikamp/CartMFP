...
if need_batches:
    ...
    with ExitStack() as stack:
        ...
        for ib, b in enumerate(bm):
            ...
            #test.append(np.sum(zm[umparts]*mdf.loc[edf.index].values.T,axis=1)*mass_blowup) #test
    ...
    # test=np.vstack(test).astype(int)      
    # fig,ax=plt.subplots()
    # for i in range(test.shape[1]):
    #     plt.plot(test[:,i])
    # ax.hlines(zmass_ixs,0,len(test))
    ...
    prev_mass,prev_comps=[],[]
    cur_ixs=0 
    borders=[]
    # cmass,cur_mass,cur_ixs=np.array([0]),np.array([0]),0
 ...
 #%%#%%
        for p in np.arange(partitions):
            ...
            #test
            borders.append([len(comps),cur_ixs,prev_ixs])            
    #%%
            # # test            
            # borders.append([m[s[0]],m[s[-1]],cur_mass[0],prev_mass[-1]])
            # print(borders[-1])
...
#%% test these should all be the same

print(total_comps)
print(zs.astype(np.int64).sum())
print(len(np.load(comp_output_path, mmap_mode="r")))
