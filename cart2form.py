...
composition_file= str(Path(basedir,                         #Required: cartesian output file generated by space2cart.py
                           "Cart_Output","H[200]C[75]N[50]O[50]P[10]S[10]_b100000max1000rdbe-5_80.0_comp.npy")) 
mass_index_file = ""
mass_defect_file = "" 
...
def predict_formula(
    ...
    # l=np.arange(t-td,t)
    # x=np.arange(t,t+td+1)
    
    # fig,ax=plt.subplots()
    # plt.plot((r/mass_blowup-933.52929)/933.52929*1e6,np.hstack([np.cumsum(emp[l][::-1][:,1])[::-1],np.cumsum(emp[x][:,1])]))
    # ax.hlines(20,-5,5,color="grey",linestyle="--")
    
    
    # plt.title(933.52929)
    # plt.xlabel("Mass error [ppm]")
    # plt.ylabel("total number of candidates")  
    # plt.legend(["candidates","top candidates cutoff"])
