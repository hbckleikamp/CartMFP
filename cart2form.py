...
# # #test
# composition_file= str(Path(basedir,                         #Required: cartesian output file generated by space2cart.py
#                             "Cart_Output","H[200]C[75]N[50]O[50]P[10]S[10]_b100000max1000rdbe-5_80_comp.npy")) 
# mode="neg"
# adducts=["-H+","+-"]
# add_formula=True
...
def predict_formula(
    ...
    if os.path.exists(params_file) and use_params:
        ...
        jdf=json.load(open(params_file))
        fs,mass_blowup,cart_max_mass,cart_rdbe_min,cart_rdbe_max=[jdf[i] for i in ["composition","mass_blowup","max_mass","min_rdbe","max_rdbe"]]
    ...
    d=np.ceil(pmi*ppm/1e6).astype(int)
    ...
    #rare case: loss adducts (like -H+): check that compositions have lost adduct.
    if acomps.values.min()<0: 
        neg_adducts=acomps[(acomps<0).any(axis=1)].index.values
        exclude=[]
        for neg_adduct in neg_adducts:
            neg_els=acomps.loc[neg_adduct,:]
            neg_els=neg_els[neg_els<0]
            neg_us=mass_df[mass_df.adduct==neg_adduct].index.values
            qneg=np.in1d(us,neg_us)
            for k,v in neg_els.items():
                eix=np.argwhere(elements==k)[0]
                exclude.append(np.argwhere(qneg & (cs[:,eix]<int(v*-1)).flatten()))
        q=~np.in1d(np.arange(len(cs)),np.hstack(exclude).flatten())
        cs,us,cq=cs[q],us[q],cq[q]
