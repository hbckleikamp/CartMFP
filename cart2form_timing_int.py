...
composition_file="C:/CartMFP/Timing_scripts/CartMFP/Cart_Output/H[400]C[150]N[100]O[100]P[20]S[20]_b40000max2000rdbe-5_140.0_comp.npy"
mode=""

#test
mode=""  
composition_file=r"c:\cartmfp\timing_scripts\cartmfp\Cart_Output\H[0,40]C[0,15]N[0,10]O[0,10]P[0,2]S[0,2]_b40000max200rdbe-5_32.0_comp.npy" 
input_file=r"c:\cartmfp\Test_masses\r0_m200_s10.npy"  
adducts=""
...
input_file=r"c:\cartmfp\Test_masses\r0_m1000_s10000.npy" 
composition_file=r"C:/CartMFP/Timing_scripts/CartMFP/Cart_Output/H[200]C[75]N[50]O[50]P[10]S[10]_b40000max1000rdbe-5_80_comp.npy"
...
# input_file=r"c:\cartmfp\Test_masses\r0_m2000_s10000.npy" 
# composition_file="C:/CartMFP/Timing_scripts/CartMFP/Cart_Output/H[400]C[150]N[100]O[100]P[20]S[20]_b40000max2000rdbe-5_140.0_comp.npy"
# mode=""
...
# x=emp[pmi,1]>top_candidates
# xum,xa_ix=pmi[x],np.argwhere(x)[:,0]

# x=emp[pmi,1]>top_candidates
# xum=(pmi[x].reshape(-1,1)+np.arange(-1,2)).flatten()
# xa_ix=np.repeat(np.argwhere(x),3)
# pmi,peak_mass_low,peak_mass_high=pmi[~x],peak_mass_low[~x],peak_mass_high[~x]
...
w=(d/2).astype(int)+1
group_ixs=np.hstack([0,np.cumsum(w)[:-1]])

l=(np.repeat(peak_mass_low,w)+(np.arange(w.sum()) - np.repeat(np.cumsum(w)-w, w))[::-1]).astype(np.uint64)
u=(np.repeat(pmi          ,w)+(np.arange(w.sum()) - np.repeat(np.cumsum(w)-w, w))).astype(np.uint64)
s=emp[l,1]+emp[u,1] #+emp[pmi,1] #reduceat cumsum
s[group_ixs]+=emp[pmi,1]

cuml=np.cumsum(s)
cuml=~(cuml-np.repeat(cuml[group_ixs],w)>top_candidates)
v=numpy_argmax_reduceat(cuml,group_ixs)+1
d=v*2+2

um=pmi.repeat(d)+(np.repeat(-v-1,d)+(np.arange(d.sum()) - np.repeat(np.cumsum(d)-d, d)))
a_ix=np.repeat(np.arange(len(masses)),d)


# a_ix=np.repeat(np.arange(len(masses[~x])),d)

# um,a_ix=np.hstack([um,xum]).astype(np.uint64),np.hstack([a_ix,xa_ix])
...
# #masses that are above top candidates
# x=emp[pmi,1]>top_candidates
# umx=(pmi[x].reshape(-1,1)+np.arange(-1,2)).flatten().astype(np.uint64)
# x_a_ix=np.repeat(np.argwhere(x),3)
# pmi,peak_mass_low,peak_mass_high=pmi[~x],peak_mass_low[~x],peak_mass_high[~x]


# #trim remaining masses
# d=(peak_mass_high-peak_mass_low)
# um=(np.repeat(peak_mass_low,d)+(np.arange(d.sum()) - np.repeat(np.cumsum(d)-d, d))).astype(np.uint64)
# a_ix=np.repeat(np.arange(len(masses))[~x],d)

# #remove nonexisting masses
# q=emp[um,1]>0
# a_ix,um=a_ix[q],um[q]


# if pre_filter_mass:
#     cl=find_closest(um,pmi)
#     br=np.argmax(np.cumsum(emp[um[np.clip(cl.reshape(-1,1) +np.arange(0, top_candidates+1   ),0,len(um)-1)],1],axis=1)>top_candidates,axis=1)+1 #right border
#     bl=np.argmax(np.cumsum(emp[um[np.clip(cl.reshape(-1,1) +np.arange(0,-top_candidates-1,-1),0,len(um)-1)],1],axis=1)>top_candidates,axis=1)+1 #left border
    
#     #clip
#     bl=np.clip(um[cl]-bl,peak_mass_low,None).astype(int)
#     br=np.clip(um[cl]+br,None,peak_mass_high).astype(int)
#     d=br-bl+1
    
#     q=np.clip(np.repeat(cl-bl,d)+(np.arange(d.sum()) - np.repeat(np.cumsum(d)-d, d)),0,len(um)-1).astype(np.uint64)
#     a_ix,um=a_ix[q],um[q]
    
# um,a_ix=np.hstack([um,umx]),np.hstack([a_ix,x_a_ix])
...
#     l_m=l_m[emp[l_m[:,0],1]>0] #remove non-existing
#     group_ixs=np.hstack([0,np.argwhere(l_m[1:,1]!=l_m[:-1,1])[:,0]+1,len(l_m)])
#     l_m=l_m[create_ranges(np.vstack([(np.vstack([group_ixs[:-1],group_ixs[1:]-top_candidates-1]).T).max(axis=1),group_ixs[1:]]).T)][::-1] #reverse order
    
#     #cumsum trim
#     group_ixs=np.hstack([0,np.argwhere(l_m[1:,1]!=l_m[:-1,1])[:,0]+1,len(l_m)])
#     cuml=np.cumsum(emp[l_m[:,0],1])
#     cuml=~(cuml-np.repeat(cuml[group_ixs[:-1]],np.diff(group_ixs))>top_candidates)
#     l_m=l_m[create_ranges(np.vstack([group_ixs[:-1],np.vstack([group_ixs[:-1]+numpy_argmax_reduceat(cuml,group_ixs[:-1]),group_ixs[1:]]).T.min(axis=1)+2]).T)]
...
#     r_m=r_m[emp[r_m[:,0],1]>0] #remove non-existing
#     group_ixs=np.hstack([0,np.argwhere(r_m[1:,1]!=r_m[:-1,1])[:,0]+1,len(r_m)])
#     r_m=r_m[create_ranges(np.vstack([group_ixs[:-1],(np.vstack([group_ixs[:-1]+top_candidates,group_ixs[1:]]).T).min(axis=1),]).T)]
    
#     #cumsum trim
#     group_ixs=np.hstack([0,np.argwhere(r_m[1:,1]!=r_m[:-1,1])[:,0]+1,len(r_m)])
#     cumr=np.cumsum(emp[r_m[:,0],1])
#     cumr=~(cumr-np.repeat(cumr[group_ixs[:-1]],np.diff(group_ixs))>top_candidates)
#     r_m=r_m[create_ranges(np.vstack([group_ixs[:-1],np.vstack([group_ixs[:-1]+numpy_argmax_reduceat(cumr,group_ixs[:-1])+1,group_ixs[1:]]).T.min(axis=1)+2]).T)]
...
filtering_mem=tracemalloc.get_traced_memory()[1]
tracemalloc.reset_peak()
...
print(time.time()-pick_best_time)
...
[filtering_time,filtering_mem/1e9],
...
performance.to_csv("performance_cart2form.tsv",sep='\t')
