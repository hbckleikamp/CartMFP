...
comps = np.load(comp_output_path, mmap_mode="c")
emp = np.load(m2g_output_path, mmap_mode="c")
...
if need_batches:
    ...
    for ib, b in enumerate(bm):
        ...
        total_filtering_time+=time.time()-filtering_time
        max_filtering_mem.append(tracemalloc.get_traced_memory()[1])
        tracemalloc.reset_peak()

        ### Pre-trim best candidates
        tracemalloc.start() 
        pick_best_time=time.time()
        ...
        # xtrim=np.arange(len(q)) #test
        
        total_pick_best_time+=time.time()-pick_best_time
        max_pick_best_mem.append(tracemalloc.get_traced_memory()[1])
        tracemalloc.reset_peak()
        ...
        # #test
        # #orig=pmi[np.repeat(qa_ix,x[:,2].astype(int))].astype(np.int64)[qr][xtrim] 
        # orig=pmi[np.repeat(qa_ix,x[:,2].astype(int))].astype(np.int64)[xtrim] 
        # pred=np.sum(c1*edf.mass.values,axis=1)
...
mfp_mem=tracemalloc.get_traced_memory()[1]
tracemalloc.reset_peak()
...
#int-based pick best filter
# print("Picking best "+str(top_candidates)+" candidates.")
# ss=np.argsort(us)
# cs,us=cs[ss],us[ss]

# ms=np.round(np.sum(cs*mdf.loc[edf.index].values.T,axis=1)*mass_blowup,0).astype(np.int64)    #float
# mdiff=abs(ms.astype(np.int64)-pmi[us])

# group_ixs=np.hstack([0,np.argwhere(us[:-1]<us[1:])[:,0]+1,len(cs)])
...
#     sl=mdiff[i[0]:i[1]+1]
#     bn=np.bincount(sl)
#     xs.append(np.argwhere(sl<=np.argmax(np.cumsum(bn)>top_candidates))[:,0]+i[0])
...
pick_best_time=time.time()-pick_best_time
pick_best_mem=tracemalloc.get_traced_memory()[1]
tracemalloc.reset_peak()
...
combine_mem=tracemalloc.get_traced_memory()[1]
tracemalloc.reset_peak()
...
# res.to_csv(MFP_outpath,sep="\t", index=False)
...
write_mem=tracemalloc.get_traced_memory()[1]

#%% Performance

performance=pd.DataFrame(
[[read_time,read_mem/1e9],
[cartesian_time,cartesian_mem/1e9],
[mfp_time,mfp_mem/1e9],
[total_filtering_time,max(max_filtering_mem)/1e9],
[total_pick_best_time,max(max_pick_best_mem)/1e9],
[combine_time,combine_mem/1e9],
[write_time,write_mem/1e9]],columns=["time (s)","peak memory (Gigabytes)"])

performance.index=["read input masses","load/construct cartesian","molecular formula prediction",
                   "chemical filtering","pick best","combine with input","write result"]
performance.loc["total",:]=[performance["time (s)"].sum(),performance["peak memory (Gigabytes)"].max()]

performance.to_csv("performance_mass2form.tsv",sep='\t')